...
positive variable proportion(comp,pool,t);
...
* Add this
Equation PoolCovTot(pool,t);
PoolCovTot(pool,t)$(ord(t)<card(t)).. I(pool,t+1) =e= I(pool,t) + sum(comp,x(comp,pool,t)) - sum(pro,y(pool,pro,t));

* Add this
Equation InvRLT(pool,t);
InvRLT(pool,t).. sum(comp,Is(comp,pool,t)) =e= I(pool,t);

* Add this
Equation FlowRLT(pool,pro,t);
FlowRLT(pool,pro,t).. sum(comp,ys(comp,pool,pro,t)) =e= y(pool,pro,t);

* Add this
Equation InvProportion;
InvProportion(comp,pool,t).. Is(comp,pool,t) =e= I(pool,t)*proportion(comp,pool,t);

* Add this
Equation FlowProportion;
FlowProportion(comp,pool,pro,t).. ys(comp,pool,pro,t) =e= y(pool,pro,t)*proportion(comp,pool,t);

* Add this
Equation PropSimplex(pool,t);
PropSimplex(pool,t).. sum(comp,proportion(comp,pool,t)) =e= 1;
...
* Add this
Equation OutFlowLogic(pool,pro,t);
OutFlowLogic(pool,pro,t).. y(pool,pro,t) =l= Xi(pool,pro)*z(pool,pro,t);
...
Model M1_MINLP
    /StreamCap,PoolCap,PoolCov,
*    OutFlowS,RSimplex,RON1,
    PoolCovTot,InvRLT,FlowRLT,
    InvProportion,FlowProportion,PropSimplex,
    arc_cap,
*    OutFlowRLT2,
    OutFlowSpec,Max_Demand,OptLogic,obj,OutFlowLogic/;

* keep this - add prop spec as well
...
* Add this
Equation Prop_BigM(qual,pool,pro,t);
Prop_BigM(qual,pool,pro,t).. sum(comp,sigma(qual,comp)*proportion(comp,pool,t)) =l= sigma_U(qual,pro) + (smax(comp,sigma(qual,comp)) - sigma_U(qual,pro))*(1 - z(pool,pro,t));


Model M1_BigM /M1_MINLP,Spec_BigM,Prop_BigM/;

positive variable v(comp,pool,pro,t),u(comp,pool,pro,t);

Equation V_def(pool,pro,t);
V_def(pool,pro,t).. sum(comp,v(comp,pool,pro,t)) =l= z(pool,pro,t);

Equation U_def(pool,pro,t);
U_def(pool,pro,t).. sum(comp,U(comp,pool,pro,t)) =l= (1 - z(pool,pro,t));
...
* Add this
Equation Proportion_Reform(comp,pool,pro,t);
Proportion_Reform(comp,pool,pro,t).. proportion(comp,pool,t) =e= v(comp,pool,pro,t) + u(comp,pool,pro,t);

* Add this
Equation ys_Reform(comp,pool,pro,t);
ys_Reform(comp,pool,pro,t).. ys(comp,pool,pro,t) =e= v(comp,pool,pro,t)*y(pool,pro,t);

Model M2_V
    /StreamCap,PoolCap,PoolCov,
*    RSimplex,RON1,
    PoolCovTot,InvRLT,FlowRLT,
    InvProportion,PropSimplex,
    arc_cap,
*    OutFlowRLT2,
    OutFlowSpec,obj,V_def,U_def,
*    I_Reform,ys_Reform,
    Proportion_Reform,ys_Reform,
    Max_Demand,OptLogic/;
...
V_UP(comp,pool,pro,t)$(Max_V(comp,pro)<1).. v(comp,pool,pro,t) =l= Max_V(comp,pro);

Equation V_RLT_GE(comp,pool,pro,t);
V_RLT_GE(comp,pool,pro,t)$(Max_V(comp,pro)<1).. ys(comp,pool,pro,t) =g= Max_V(comp,pro)*sum(compp,ys(compp,pool,pro,t)) + v(comp,pool,pro,t)*Xi(pool,pro) - Max_V(comp,pro)*Xi(pool,pro);
...
Model M6_RT /M2_VSpec,V_UP,V_RLT_GE/;
...
Solve M6_RT using minlp maximizing profit;
