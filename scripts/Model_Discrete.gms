...
parameter d number of discrete point /100/;
set M /M1*M100/;
parameter psi(m);
...
Equation Eqn_11_34;
Eqn_11_34(j,p,t,m).. Z_minus(j,p,t,m) + Z_plus(j,p,t,m) =e= z_dis(j,p,t,m);
...
Eqn_11_40(s,j,p,t).. v(s,j,p,t) =e= sum(m$(ord(m)<d),psi(m)*W_vm(s,j,p,t,m)) + sum(m$(ord(m)<d), (psi(m+1) - psi(m))*W_vm_plus(s,j,p,t,m));

Equation Eqn_11_40_I replacing 11_28_I in Rex;
Eqn_11_40_I(s,j,p,t).. Is(s,j,t) =e= sum(m$(ord(m)<d),psi(m)*W_vm(s,j,p,t,m)) + sum(m$(ord(m)<d), (psi(m+1) - psi(m))*W_vm_plus(s,j,p,t,m));
...
solve M_Dis using mip maximizing profit;
